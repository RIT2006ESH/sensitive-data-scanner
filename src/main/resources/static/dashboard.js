...
async function loadDataTypes() {
    try {
        ...
        types.forEach(t => {
            ...
            regexRow.className = 'pii-regex-row';
            regexRow.hidden = true;
            regexRow.innerHTML = `<input type="text" class="pii-regex-input" data-type="${t}" placeholder="Custom regex (optional) — overrides default pattern for ${friendlyType(t)}">
        <span class="pii-regex-error" hidden>Invalid regex</span>`;
            row.appendChild(regexRow);

            group.appendChild(row);
        });

        document.querySelectorAll('.pii-regex-input').forEach(input => {
...
function togglePiiChip(chip, type) {
    ...
    const regexRow = chip.closest('.pii-chip-row').querySelector('.pii-regex-row');

    if (idx > -1) {
        selectedPiiTypes.splice(idx, 1);
        chip.classList.remove('selected');
        if (regexRow) regexRow.hidden = true;
    } else {
        selectedPiiTypes.push(type);
        chip.classList.add('selected');
        if (regexRow) regexRow.hidden = false;
    }
    updatePiiDropdownLabel();
}

function updatePiiDropdownLabel() {
    const label = document.getElementById('pii-dropdown-label');
    if (!label) return;
    if (selectedPiiTypes.length === 0) {
        label.textContent = 'All PII Types';
    } else if (selectedPiiTypes.length === 1) {
        label.textContent = friendlyType(selectedPiiTypes[0]);
    } else {
        label.textContent = `${selectedPiiTypes.length} PII Types selected`;
    }
}

/* ---------- PII dropdown open/close ---------- */
const piiDropdown = document.getElementById('pii-dropdown');
const piiDropdownToggle = document.getElementById('pii-dropdown-toggle');
const piiDropdownPanel = document.getElementById('pii-dropdown-panel');

if (piiDropdownToggle) {
    piiDropdownToggle.addEventListener('click', () => {
        const isOpen = !piiDropdownPanel.hidden;
        piiDropdownPanel.hidden = isOpen;
        piiDropdownToggle.classList.toggle('open', !isOpen);
    });
}

document.addEventListener('click', (e) => {
    if (piiDropdown && !piiDropdown.contains(e.target)) {
        piiDropdownPanel.hidden = true;
        piiDropdownToggle.classList.remove('open');
    }
});

const piiSelectAllBtn = document.getElementById('pii-select-all-btn');
if (piiSelectAllBtn) {
    piiSelectAllBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        document.querySelectorAll('#pii-chip-group .pii-chip-row').forEach(row => {
            const chip = row.querySelector('.pii-chip');
            const regexRow = row.querySelector('.pii-regex-row');
            const type = chip.dataset.type;
            if (!selectedPiiTypes.includes(type)) {
                selectedPiiTypes.push(type);
                chip.classList.add('selected');
            }
            if (regexRow) regexRow.hidden = false;
...
const piiClearBtn = document.getElementById('pii-clear-btn');
if (piiClearBtn) {
    piiClearBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        selectedPiiTypes = [];
        customPatterns = {};
        document.querySelectorAll('#pii-chip-group .pii-chip-row').forEach(row => {
            row.querySelector('.pii-chip').classList.remove('selected');
            const regexRow = row.querySelector('.pii-regex-row');
            const input = row.querySelector('.pii-regex-input');
            if (regexRow) regexRow.hidden = true;
            if (input) {
                input.value = '';
                input.classList.remove('invalid');
...
function triggerScan() {
    const invalidInputs = document.querySelectorAll('.pii-regex-input.invalid');
    if (invalidInputs.length > 0) {
        alert('Fix the invalid custom regex pattern(s) before running the scan.');
        return;
    }
    ...
    selectedPiiTypes.forEach(type => {
        if (customPatterns[type]) relevantPatterns[type] = customPatterns[type];
    });
    if (Object.keys(relevantPatterns).length > 0) payload.customPatterns = relevantPatterns;
